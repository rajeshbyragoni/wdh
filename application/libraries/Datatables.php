<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
 ...
 //$this->having('table1.second_id = table2.fi_second_id',false);
	...
	/**/
	 public function order_by($column,$action)

    {

      $this->order_by = $column;

      $this->ci->db->order_by($column,$action);
   ...
   /* public function like($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
	  ...
	  //$this->ci->db->where($key_condition.' LIKE', '%'.$val.'%');

	
      $this->ci->db->like($key_condition, $val, $backtick_protect);
	  
	 
	 //$aColumns[$i]." LIKE '%".mysql_real_escape_string( $_GET['sSearch'] )."%' OR ";
	...
	/*
	 ...
	 $sWhere = '';
      ...
      $sWhere .= ($key_condition . " LIKE '%" . $val . "%'");
	  
				  $key_condition.' LIKE', '%'.$val.'%'
    ...
    protected function produce_output($charset)
    {
...

      {
        ...
        if(!$this->check_mDataprop())
          ...
          if($this->table==$this->ci->tableName)
		  {
			 			  
			  if(isset($this->ci->editActionTitle) && $this->ci->editActionTitle!=''){
					$EditTitle = $this->ci->editActionTitle;
				}else{
					$EditTitle = 'Edit';
				}
			  
			 if($this->ci->statusFlag == true){ 
				 if($aaData[$row_key][$this->ci->statusColumnNO]=="INACTIVE"){
					$aaData[$row_key][$this->ci->actionColumnNO]= '<div class="actionwrp">
												<label class="switch-light switch-ios" id="ip_status'.$aaData[$row_key][$this->ci->idColumnNo].'" style="width: 100px;">
													<input type="hidden" value="'.$aaData[$row_key][$this->ci->statusColumnNO].'" id="alert_status_'.$aaData[$row_key][$this->ci->idColumnNo].'" />
													<input onclick="myFunction(this)" class="role1" id="role1_'.$aaData[$row_key][$this->ci->idColumnNo].'" data-alert_id1="'.$aaData[$row_key][$this->ci->idColumnNo].'" type="checkbox"/>
													<span class="htspan"> 
														<span>INACTIVE</span>
														<span>ACTIVE</span>
													</span>
													<a><img id="role_load1'.$aaData[$row_key][$this->ci->idColumnNo].'"  width="26"  style="display: none;"  src="'.base_url().'/images/loader1.gif"></a>
												</label>                                            
											</div>';
								if($this->ci->editFlag == true){							
								$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="actionwrp"><a class="btn btn-info" href="'.base_url().$this->ci->editActionurl.'/'.base64_encode(json_encode($aaData[$row_key][$this->ci->idColumnNo])).'">'.$EditTitle.'</a></div>';
								}			
								if($this->ci->deleteFlag == true){
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="actionwrp"><a class="btn btn-danger" onClick="return confirm(`Are you sure want to delete this records?`);" href="'.base_url().$this->ci->deleteActionurl.'/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-remove"></span>Delete</a></div>';
								}
								if(isset($this->ci->EditUser) && $this->ci->EditUser == true){
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="actionwrp"><a class="btn btn-info" href="'.base_url().$this->ci->EditUserLInk.'/'.base64_encode(json_encode($aaData[$row_key][0])).'">'.$this->ci->EditUserTitle.'</a></div>';
								}

								if($this->ci->OtherFlag == true){
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="actopn successbtn btn" href="'.base_url().'/'.$this->ci->otherFlagTitleLink.'/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>'.$this->ci->otherFlagTitle.'</a></div></div>';
								}

								if(isset($this->ci->subUserColumnNO) && $this->ci->subUserColumnNO!=''){
									if($this->table == 'passenger_details'){
										$aaData[$row_key][$this->ci->subUserColumnNO] ='<div class="allaction"><div class="actionwrp"><a class="actopn successbtn btn" href="'.base_url().'/passenger/passenger_list/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-user"></span>View Sub Passenger</a></div></div>';
									}
								}
								
								if($this->ci->manageFlag == true){
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="btn-warning btn" href="'.base_url().'/'.$this->ci->manageUserType.'/deposits/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Deposits</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="actopn logbtn btn" href="'.base_url().'/'.$this->ci->manageUserType.'/saved_quote/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Saved Quote</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="btn btn-danger" href="'.base_url().'/'.$this->ci->manageUserType.'/reservation_list/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Reservation List</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="btn-primary btn" href="'.base_url().'/'.$this->ci->manageUserType.'/send_email/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Send Email</a></div></div>';
								}
					...
					$aaData[$row_key][$this->ci->actionColumnNO]= '<div class="actionwrp">
												<label class="switch-light switch-ios" id="ip_status'.$aaData[$row_key][$this->ci->idColumnNo].'" style="width: 100px;">
													<input type="hidden" value="'.$aaData[$row_key][$this->ci->statusColumnNO].'" id="alert_status_'.$aaData[$row_key][$this->ci->idColumnNo].'" />
													<input  onclick="myFunction(this)" class="role1" id="role1_'.$aaData[$row_key][$this->ci->idColumnNo].'" data-alert_id1="'.$aaData[$row_key][$this->ci->idColumnNo].'" type="checkbox" checked="checked"/>
													<span class="htspan"> 
														<span>INACTIVE</span>
														<span>ACTIVE</span>
													</span>
								...
								if($this->ci->manageFlag == true){
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="btn-warning btn" href="'.base_url().'/'.$this->ci->manageUserType.'/deposits/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Deposits</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="actopn logbtn btn" href="'.base_url().'/'.$this->ci->manageUserType.'/saved_quote/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Saved Quote</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="actopn delbtn  btn" href="'.base_url().'/'.$this->ci->manageUserType.'/reservation_list/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Reservation List</a></div></div>';
									$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="allaction"><div class="actionwrp"><a class="btn-primary btn" href="'.base_url().'/'.$this->ci->manageUserType.'/send_email/'.base64_encode(json_encode($aaData[$row_key][0])).'"><span class="fa fa-check-square"></span>Send Email</a></div></div>';
								}
				}
			}else{
					$aaData[$row_key][$this->ci->actionColumnNO] = '';
					if($this->ci->editFlag == true){							
						$aaData[$row_key][$this->ci->actionColumnNO] .='<div class="actionwrp"><a class="btn btn-info" href="'.base_url().$this->ci->editActionurl.'/'.base64_encode(json_encode($aaData[$row_key][$this->ci->idColumnNo])).'">'.$EditTitle.'</a></div>';
					}			
					if($this->ci->deleteFlag == true){
